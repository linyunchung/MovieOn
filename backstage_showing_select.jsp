						...
						<form method="post"
							...
							選擇影城 <select name="theaterId">
								...
								<c:forEach var="theaterVO" items="${theaterSvc.all}">
									<option value="${theaterVO.theaterId }"${(theaterId==theaterVO.theaterId)? 'selected' : '' }>${theaterVO.theaterName}
								</c:forEach>
							</select> 選擇電影 <select name="movieId">

								<c:forEach var="movieVO" items="${movieSvc.all }">
									<option value="${movieVO.movieId }"${(movieId==movieVO.movieId)? 'selected' : '' }>${movieVO.movieName}
								</c:forEach>
							</select>
