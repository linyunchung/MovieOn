		...
		<div class="content-wrap">
			<div class="profile_header">
				<div class="profile_summary">
					<div class="profile-avatar">
						<span class="avatar" id="avatar"> <img
							src="${pageContext.request.contextPath}/DBGifReaderFollow?userid=${id}"
							alt="里維阿卡曼兵長" width="100" height="100">
						</span>
					</div>
					...
					<div class="profile-info">
						<div class="profile-social">
							<a class="account" href=""> <i class="fa fa-instagram"></i> <span>${memSvc.getOneMember(id).getIg()}</span>
							</a> <a class="account" href=""> <i class="fa fa-facebook"></i> <span>Facebook</span>
							</a>
						</div>
						<div class="profile-stats">
							<h4 class="profile-statistic">
								<a
									...
									<span class="value">${rvwSvc.userReviewCount(id)}</span> <span
									class="definition">看過</span>
								</a>
							...
							<h4 class="profile-statistic">
								<a
									href="${pageContext.request.contextPath}/profile?id=${id}&action=reviews">
									<span class="value">${rvwSvc.userReviewCount(id)}</span> <span
									class="definition">影評</span>
								</a>
							...
							<h4 class="profile-statistic">
								<a
									...
									<span class="value">${followService.followerCount(id)}</span> <span
									class="definition">粉絲</span>
								</a>
						...
						<div class="profile-button">
							<c:choose>
								<c:when test="${not empty followVO}">
									<a class="button"
										onclick="updateFollow('removeFollow');return false;">取消追蹤</a>
								</c:when>
								<c:otherwise>
									<c:choose>
										<c:when test="${param.id==loginMemberId}">
											<a class="button"
												href="${pageContext.request.contextPath}/member/profile.jsp">編輯個人檔案</a>
										</c:when>
										<c:otherwise>
											<a class="button nofollow"
												onclick="updateFollow('addFollow');return false;">追蹤</a>
										</c:otherwise>
									</c:choose>
								</c:otherwise>
							</c:choose>
				...
				<nav class="profile-navigation">
					<ul class="navlist">
						<li class="navitem"><a class="navlink navuserid"
						...
						<li class="navitem"><a class="navlink"
							href="${pageContext.request.contextPath}/profile?id=${id}&action=following">
								追蹤中 </a></li>
						<li class="navitem"><a class="navlink"
							href="${pageContext.request.contextPath}/profile?id=${id}&action=network">
								動態牆 </a></li>
						<c:if test="${param.id==loginMemberId}">
							<li class="navitem"><a class="navlink"
								href="<%=request.getContextPath()%>/shop/shopSearch.jsp">
			...
			<div class="left-col">
								...
								<div class="poster">
									<div>
										<img class="image"
											...
											alt="${movieSvc.getOneMovie(reviewVO.movieId).getMovieName()}">
										<a class="frame"
											href="Links_Controller?movieId=${reviewVO.movieId}&action=getOneMovie_From_Home">
				...
				<section class="section">
					<h2 class="section-h2">
						<a
							href="${pageContext.request.contextPath}/profile?id=${id}&action=reviews">最新影評</a>
					</h2>
					<a
						href="${pageContext.request.contextPath}/profile?id=${id}&action=reviews"
						class="all-link">更多</a>
							...
							<li class="film-detail film-watched">
								<div class="poster film-poster poster-container">
									<div>
										<img class="image"
											src="${pageContext.request.contextPath}/DBGifReaderProfile?movieId=${reviewVO.movieId}"
											width="70" height="105" alt="I Kill Giants"> <a
											class="frame"
											href="Links_Controller?movieId=${reviewVO.movieId}&action=getOneMovie_From_Home">
											<span class="frame-title">黑寡婦 (2021)</span> <span
											class="overlay"></span>
										</a>
									</div>
								</div>
								<div class="film-detail-content">
									<h2 class="headline-2">
										<a
											href="Links_Controller?movieId=${reviewVO.movieId}&action=getOneMovie_From_Home">${movieSvc.getOneMovie(reviewVO.movieId).getMovieName()}</a>
										<small class="metadata"> <a href="">${rvwSvc.getReleaseYear(reviewVO)}</a>
										</small>
									</h2>
									<div class="attribution-block">
										<p class=attribution>
											<span class="rating -blue rated-5">${reviewVO.starRate}</span>
											<span class="content-metadata"> <span class="date">
													<a class="context" href=""> Watched by <strong
														class="name">${memSvc.getOneMember(reviewVO.userId).getUsername()}</strong>
												</a> <span class="_nobr">${rvwSvc.getYearMonthDate(reviewVO.postedAt)}</span>
											</span>
											</span>
										</p>
									</div>
									...
									<p class="like-link-target">
										<span class="like-link"> <a href=""> <i
												class="fa fa-heart"></i> <span>還沒有人按讚</span>
										</a>
										</span>
									</p>
								</div>
				...
				<section class="section">
					<h2 class="section-h2">
						<a
							href="${pageContext.request.contextPath}/profile?id=${id}&action=reviews">熱門影評</a>
					</h2>
					<a
						href="${pageContext.request.contextPath}/profile?id=${id}&action=reviews"
						class="all-link">更多</a>
					<ul class="film-details-list poster-list">
						<li class="film-detail film-watched">
							<div class="poster film-poster poster-container">
								<div>
									<img class="image"
										src="https://movies.yahoo.com.tw/i/o/production/movies/July2021/tDAKOcY2jZvGxxZI3gcp-1024x1463.jpg"
										width="70" height="105" alt="I Kill Giants"> <a
										class="frame" href=""> <span class="frame-title">玩命關頭9
											(2021)</span> <span class="overlay"></span>
									</a>
								</div>
							</div>
							<div class="film-detail-content">
								<h2 class="headline-2">
									<a href="">玩命關頭9</a> <small class="metadata"> <a
										href="">2021</a>
									</small>
								</h2>
								<div class="attribution-block">
									<p class=attribution>
										<span class="rating -blue rated-9">★★★★½</span> <span
											class="content-metadata"> <span class="date"> <a
												class="context" href=""> Watched by <strong class="name">里維阿卡曼兵長</strong>
											</a> <span class="_nobr">2021-07-09</span>
										</span>
										</span>
									</p>
								</div>
						...
						<li class="film-detail film-watched">
							...
							<div class="poster film-poster poster-container">
								<div>
									<img class="image"
										src="https://movies.yahoo.com.tw/i/o/production/movies/July2020/hFH7arAJqBIZaUdOGiuC-1182x1691.jpg"
										width="70" height="105" alt="I Kill Giants"> <a
										class="frame" href=""> <span class="frame-title">鬼滅之刃劇場版
											無限列車篇 (2021)</span> <span class="overlay"></span>
									</a>
								</div>
							</div>
							<div class="film-detail-content">
								<h2 class="headline-2">
									<a href="">鬼滅之刃劇場版 無限列車篇 </a> <small class="metadata">
										<a href="">2020</a>
									</small>
								</h2>
								<div class="attribution-block">
			...
			<aside class="adsdisplay">
				<div>
					<img
						src="https://tpc.googlesyndication.com/simgad/10044394848394388336"
						alt="">
				</div>
				<div>
					<img
						src="https://tpc.googlesyndication.com/simgad/14451398280381465866"
						alt="">
				</div>
				<div>
					<img
						src="https://tpc.googlesyndication.com/simgad/13972977176807790839"
						alt="">
				</div>
			</aside>
		</div>
