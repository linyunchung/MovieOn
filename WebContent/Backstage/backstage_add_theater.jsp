...
<body>
	...
	<div class="content">
		...
		<div class="aside">
			...
			<div class="aside_search">
				...
				<a class="aside_btn" href="<%=request.getContextPath()%>/Backstage/backstage_add_theater.jsp">上架影城表</a> <a
					class="aside_btn aside_addtime" href="<%=request.getContextPath()%>/Backstage/backstage_add_time.jsp">新增電影時刻表</a> <a
					class="aside_btn" href="<%=request.getContextPath()%>/Backstage/backstage_showing_select.jsp">已上架管理</a>
			</div>
		</div>
    ...
    <c:if test="${successMsg.length()!=0 }">
    	<ul style="list-style-type:none">
			...
			<form method="post"
				action="<%=request.getContextPath()%>/area/area.do">
				<table>
					<tr>
						<td>新增地區:</td>
						<td><input type="TEXT" autocomplete="off" placeholder="請輸入縣市名，ex.XX市(縣)(選填)" name="city" size="10"
					...
					<tr>
						...
						<td><select size="1" name="area">
								<c:forEach var="areaVO" items="${areaSvc.all}">
									<option value="${areaVO.areaNum}"${(theaterVO.areaNum==areaVO.areaNum) ? 'selected' : '' }>${areaVO.city}
								</c:forEach>
						</select></td>
					</tr>
					...
					<tr>
						<td>影城名稱:</td>
						<td><input type="TEXT" autocomplete="off" placeholder="請輸入影城名稱(必填)" name="theaterName" size="45"
				...
				<input type="hidden" name="action" value="insert"> <input
					type="submit" value="送出新增" id="showing_time_btn">
