...
<title>訂單查詢</title>
<link rel="stylesheet" href="<%=request.getContextPath()%>/css/orderSearch.css">
	...
	<div id="wrapper">
		...
		<div id="header">
			<nav>
				...
				<ul>
					...
					<li class="dropdown"><a href="<%=request.getContextPath()%>/shop/shopPage.jsp">商城</a>
						<ul>
		...
		<main>
			<div id="content">
						...
						<c:forEach var="orderInfoVO" items="${orderInfolist}">
							<table class="order_no">
								...
								<tr>
									...
									<td><fmt:formatDate type="both" dateStyle="medium" timeStyle="medium" value="${orderInfoVO.orderDate}" /></td>
								...
								<div class="panel">
									<table class="order_item">
										...
										<c:forEach var="orderListVO" items="${orderList}">
											...
											<c:set var="itemVO" value="${itemSvc.getOneItem(orderListVO.itemId)}"></c:set>
				...
				<div class="last_btn">
					<a href="<%=request.getContextPath()%>/shop/shopPage.jsp"><button class="btn1">回到首頁</button></a> 
					<a href="<%=request.getContextPath()%>/shop/shopItem.jsp"><button class="btn2">繼續購物</button></a>
				</div>
			</div>
		</main>
		<!--Close Main-->
